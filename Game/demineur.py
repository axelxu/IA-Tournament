...
class Demineur :
    ...
    def est_valide(self, plateau, action):
        ...
        return i in range(20) and j in range(20) and not plateau.surface[i][j].etat[1]
    def next(self, plateau, action, num_tour):
        i, j = map(int, action.split())
        plateau.surface[i][j].etat[1] = True
            ...
            for k2 in [j-1, j, j+1]:
                if k1 in range(20) and k2 in range (20):
                    if not plateau.surface[k1][k2].vide:
                        nombre_mines += 1
        plateau.surface[i][j].etat[0] = nombre_mines
        if nombre_mines == 0:
            for k1 in [i-1, i, i+1]:
                for k2 in [j-1, j, j+1]:
                    if k1 in range(20) and k2 in range (20):
...
def demineur_resultat(plateau):
    for i in range(20):
        for j in range(20):
            Case = plateau.surface[i][j]
            if (Case.vide == False) and Case.etat[1]:
                    return "Perdu"
    return "Gagné"
